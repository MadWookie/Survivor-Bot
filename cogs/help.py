...
class Help(formatter.HelpFormatter):
    ...
    async def send(self, dest, content=None, embeds=None):
        ...
        while True:
            ...
            try:
                ...
                await help_msg.edit(content=content, embed=embeds[page_number], timeout=60)
            except:
                await help_msg.edit(embed=None, delete_after=15)
                await page_msg.edit(content="Quit Help menu.", delete_after=30)
                break
    ...
    async def help(self, ctx, *cmds: str):
        ...
        if not ctx.message.author.permissions_in(ctx.channel).embed_links:
            return await ctx.send(content="You don't have permissions to send embeds here. Find a different server/channel where you can embed links and try the help command there.", delete_after=30)
